...
- New work rule: any new UI buttons/endpoints/functions must ship with unit + integration + e2e + smoke tests and logging/analytics (backend logs + `AnalyticsLogger`).
- Locations: parent is optional on create, name/parent can be updated; location photo is set/cleared via `/api/v1/locations/{id}/photo`; upload history includes `location_id`.
- Multi-item hints: pass `hint_item_ids` in upload/analyze to boost candidate linking for detections.
